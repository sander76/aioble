...
from aioble.bluezdbus.bd_characteristic import (
    CharacteristicBlueZDbus as Characteristic,
)
    ...
    async def resolve_characteristics(self):
        ...
        self.characteristics = [
            Characteristic(
                service=self,
                path=objpath,
                uuid=objvalue["org.bluez.GattCharacteristic1"]["UUID"][1],
            )
            for objpath, objvalue in values.items()
            if characteristics_regex.match(objpath)
        ]
