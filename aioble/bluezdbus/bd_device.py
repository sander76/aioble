...
_device_path_regex_path = re.compile(
...
class DeviceBlueZDbus(Device):
    ...
    def __init__(self, device, loop=None):
        ...
        self.address = "/org/bluez/%s/dev_%s" % (
            "hci0",
            device.replace(":", "_").upper(),
        )
    ...
    async def connect(self):
        ...
        try:
            ...
            self._dbus = await dbus.Connection.bus_get_async(
                DBUS.BUS_SYSTEM, private=False, loop=self.loop
            )
        except:
        ...
        await self._dbus.bus_add_match_action_async(
            self.properties_rule, self.properties_changed, None
        )
    ...
    def _connect_succeeded(self):
        # Call User Callback
        if self.connect_succeeded is not None:
            self.connect_succeeded()

    def _disconnect_succeeded(self, path):
        # Call User Callback
        if self.disconnect_succeeded is not None:
                    ...
                    if message.path in self._notification_callbacks:
                        if "Value" in list(message.objects)[1]:
                            ...
                            self._notification_callbacks[message.path](
                                message.path,
                                list(message.objects)[1]["Value"][1],
                            )
    ...
    async def disconnect(self):
        ...
        await self._dbus.bus_remove_match_action_async(
            self.properties_rule, self.properties_changed, None
        )
