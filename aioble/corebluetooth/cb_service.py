...
class CoreBluetoothService(Service):
    def __init__(self, device : CoreBluetoothDevice, cbservice : CoreBluetooth.CBService, queue, *args, **kwargs):
        ...
        from aioble.corebluetooth.characteristic import CoreBluetoothCharacteristic as _characteristic
    ...
    async def discover_characteristics(self) -> List[CoreBluetoothCharacteristic]:
        ...
        if self._characteristics_by_identifier is None:
            ...
            self._characteristics_by_identifier = {cbchar.UUID().UUIDString(): CoreBluetoothCharacteristic(self, cbchar, self._queue) for cbchar in cbcharacteristics}
        return self._characteristics_by_identifier.values()

    async def characteristic_with_identifier(self, identifier : str) -> CoreBluetoothCharacteristic:
