...
class CoreBluetoothCentralManager(CentralManager):
    ...
    def __init__(self, loop=None, *args, **kwargs):
        super(CoreBluetoothCentralManager, self).__init__(
            loop, *args, **kwargs
        )
        ...
        self._cbmanager = CoreBluetooth.CBCentralManager.alloc().initWithDelegate_queue_options_(
            self, self._queue, None
        )

        # We need to defer the import of CoreBluetoothDevice due to circular dependency
        from aioble.corebluetooth.cb_device import (
            CoreBluetoothDevice as _device,
        )
    ...
    async def _notify_device_found(
    ...
    @util.dispatched_to_loop()
    async def _peripheral_did_connect(
        self, peripheral: CoreBluetooth.CBPeripheral
    ):
        device = self.devices.get(peripheral.identifier().UUIDString())
        if device:
